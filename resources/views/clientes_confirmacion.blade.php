    ...
    <div class="bg-light-texture grid lg:grid-cols-1 md:grid-cols-1 xs:grid-flow-row sm:grid-flow-row">
        <div
            ...
            <div class="relative flex flex-col min-w-0 rounded break-words border bg-white border-1 border-gray-300">
                <div class="py-3 px-6 mb-0 bg-gray-200 border-b-1 border-gray-300 text-gray-900">
                    <h1 class="text-6xl text-green-700 transition-all hover:text-green-900">
                        {{ __('Clientes Potenciales Landing Univisión') }}</h1>
                </div>
            ...
            <div class="container mx-auto">
                ...
                <a href="https://mendozafirm.com/wp-content/plugins/descargar/descargar_clientes_potenciales.php" class="mr-8 btn-green inline-block align-middle text-center select-none border mt-2.5 float-right whitespace-no-wrap rounded py-1 px-3 leading-normal no-underline text-white hover:bg-blue-600 bg-blue-500 font-bold">Descargar Registros</a><br>
                <br>
                    ...
                    <table class="w-full table-auto">
                        <thead>
                            <tr>
                                ...
                                <th data-sort="correo_ingresado" class="border px-4 py-2">Correo Ingresado</th>
                                ...
                                <th data-sort="rev" class="border px-4 py-2">Registrado en Univision</th>
                                <th data-sort="inicio" class="border px-4 py-2">Iniciado Sesión</th>
                                <th data-sort="face" class="border px-4 py-2">Tiene Facebook</th>
                        ...
                        <tbody>
                            ...
                            @endphp
                                <tr data-envio="{{ $cliente->enviado_cli ? 'No Enviado' : 'Enviado' }}">
                                    <td class="border px-4 py-2" data-type="correo">{{$data_decode['email_get'] == 'fb' ? $data_decode['email_get'] : $cliente->correo_cli}}</td>
                                    <td class="border px-4 py-2" data-type="nombre">{{ $data_decode['nombre'] }}</td>
                                    <td class="border px-4 py-2" data-type="apellido">{{ $data_decode['apellido'] }}</td>
                                    <td class="border px-4 py-2" data-type="correo_ingresado">{{ $data_decode['email'] }}</td>
                                    <td class="border px-4 py-2" data-type="mismo_correo">{{ $data_decode['same-email'] }}</td>
                                    <td class="border px-4 py-2" data-type="correo_opcional">{{ $data_decode['correo-opcional'] }}</td>
                                    <td class="border px-4 py-2" data-type="tel">{{ $data_decode['telefono'] }}</td>
                                    <td class="border px-4 py-2" data-type="rev">{{ $data_decode['univision-registered'] }}</td>
                                    <td class="border px-4 py-2" data-type="inicio">{{ $data_decode['univision-logged'] }}</td>
                                    <td class="border px-4 py-2" data-type="face">{{ $data_decode['has-facebook'] }}</td>
                                    <!--<td class="border px-4 py-2" data-type="pid">Pid:{{ isset($data_decode['pid_get']) ? $data_decode['pid_get'] : $cliente->pid_cli }}</td>-->
